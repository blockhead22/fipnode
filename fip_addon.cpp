...
Napi::ThreadSafeFunction tsfn;  // Thread-safe function for button callback

#define FLAG_SET_AS_ACTIVE 0x00000001
...
void __stdcall ButtonCallback(void* hDevice, DWORD dwButtons, void* pCtxt) {
    printf("ButtonCallback TRIGGERED! hDevice: %p, dwButtons: 0x%08X\n", hDevice, dwButtons);

    if (dwButtons == 0) {
        printf("No button press detected (dwButtons == 0).\n");
        return;
    }

    printf("🎮 BUTTON PRESS DETECTED! Button state: 0x%08X\n", dwButtons);

    // Send event to Node.js
    tsfn.NonBlockingCall([dwButtons](Napi::Env env, Napi::Function jsCallback) {
        jsCallback.Call({ Napi::Number::New(env, dwButtons) });
    });
}
...
// Register button callback for Node.js
Napi::Number RegisterButtonCallback(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    ...
    printf("Checking g_hDevice before registering callback: %p\n", g_hDevice);

    if (g_hDevice == nullptr) {
        printf(" ERROR: g_hDevice is NULL! Button callback will not work.\n");
        return Napi::Number::New(env, -1);
    }

    tsfn = Napi::ThreadSafeFunction::New(env, info[0].As<Napi::Function>(), "ButtonCallback", 0, 1);

    // Ensure that we only register the button callback if the function is available
    if (g_hDevice == nullptr) {
        printf(" ERROR: g_hDevice is NULL! Cannot register button callback.\n");
        return Napi::Number::New(env, -1);
    }
    
    HRESULT hr = g_DirectOutput.RegisterSoftButtonCallback(g_hDevice, ButtonCallback, nullptr);
    if (hr != S_OK) {
        printf(" Failed to register button callback! Error Code: 0x%08X\n", hr);
        return Napi::Number::New(env, hr);
    }

    printf("utton callback registered successfully!\n");
...
// Register functions to Node.js
Napi::Object Init(Napi::Env env, Napi::Object exports) {
    exports.Set(Napi::String::New(env, "initDirectOutput"), Napi::Function::New(env, InitDirectOutput));
    exports.Set("registerButtonCallback", Napi::Function::New(env, RegisterButtonCallback));
